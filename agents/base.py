...
class BaseAgent:
    ...
    async def handle_exception(
        ...
        order: Optional[Order] = None # Make order optional
    ) -> None:
        ...
        if order is not None and hasattr(order, 'update_status') and hasattr(order, 'order_id'):
            try:
                ...
                error_details["order_id"] = order.order_id # Add order_id if updated
                 ...
                 logger_base.error(f"Failed to update order status during exception handling for order {getattr(order, 'order_id', 'N/A')}: {update_err}")
