...
def prepare_analysis_data(
    ...
    if product_data is not None:
        ...
        analysis_df = pd.merge(
            analysis_df, product_data, on="product_id", how="left"
        )
    ...
    if store_data is not None:
        ...
        analysis_df = pd.merge(
            analysis_df, store_data, on="store_id", how="left"
        )
    ...
    if promotion_data is not None:
        ...
        if (
            "date" in promotion_data.columns
        ...
        valid_merge_keys = [
            k
            for k in merge_keys
            if k in analysis_df.columns and k in promotion_data.columns
        ]

        if not valid_merge_keys:
            warnings.warn(
                "Could not find common keys (date, product_id, store_id) to merge promotion data. "
                "Assuming no promotions applied.",
                UserWarning
            )
