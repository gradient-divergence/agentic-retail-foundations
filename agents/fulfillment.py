...
class FulfillmentAgent(BaseAgent):
    ...
    async def handle_order_allocated(self, event: RetailEvent) -> None:
        ...
        try:
            ...
            await self.publish_event(
                ...
                {"order_id": order_id, "amount": event.payload.get("order_total", 0)} # Assuming total is passed or fetchable
            )
            ...
            await self.handle_exception(exception=e, context={"stage": "payment_initiation", "order_id": order_id}, order=None) # Pass None for order if not fetched
    ...
    async def handle_payment_processed(self, event: RetailEvent) -> None:
        ...
        try:
            ...
            await self.publish_event(
                 ...
                 {"order_id": order_id, "new_status": OrderStatus.PICKING.value, "details": {"fulfillment_groups": len(fulfillment_groups)}}
            )
    ...
    async def _initiate_fulfillment_request(
        ...
        item_details = [
            {"product_id": item.product_id, "quantity": item.quantity, "item_id": getattr(item, 'id', None)} # Add item ID if available
            for item in items
        ]
