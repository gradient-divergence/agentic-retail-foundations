...
class InventoryAgent(BaseAgent):
    ...
    async def handle_order_validated(self, event: RetailEvent) -> None:
        ...
        try:
            ...
            await self.publish_event(
                "order.allocated", 
                {"order_id": order.order_id, "allocation_details": self._get_allocation_details(order)}
            )
    ...
    def _get_allocation_details(self, order: Order) -> Dict[str, Any]:
         ...
         if not hasattr(order, 'items'): return {}
         return {
             "items": [
                 {
                 ...
                 } for item in order.items if hasattr(item, 'product_id') # Basic check
             ]
         }
