...
class PromotionCausalAnalyzer:
    ...
    def _define_causal_graph(
        ...
        if common_causes is None:
            ...
            potential_causes = [
                ...
                if col not in [treatment, outcome, "date"] # Exclude date if not used as direct feature
    ...
    def estimate_ate_doubleml_irm(
        ...
        try:
            ...
            if len(numeric_confounders) < len(x_cols):
                print(f"Warning: Non-numeric confounders excluded from DoubleML: {set(x_cols) - set(numeric_confounders)}")
            ...
            if data_subset.shape[0] < data.shape[0]:
                print(f"Warning: Dropped {data.shape[0] - data_subset.shape[0]} rows with NaNs before DoubleML.")
            ...
            if ml_learner_name.lower() == "randomforest":
                ...
                ml_g = RandomForestRegressor(n_estimators=100, max_depth=5, min_samples_leaf=5, random_state=123) # Model for E[Y|X]
                ml_m = RandomForestClassifier(n_estimators=100, max_depth=5, min_samples_leaf=5, random_state=123) # Model for E[D|X] (propensity score)
            elif ml_learner_name.lower() == "lasso":
                from sklearn.linear_model import LassoCV, LogisticRegressionCV
                ml_g = LassoCV(cv=5, random_state=123)
                ml_m = LogisticRegressionCV(cv=5, solver='liblinear', random_state=123) # Use Logistic for binary treatment
            else:
    ...
    def simulate_counterfactuals(self, model_type='causalforest', **kwargs):
        ...
        if model_type == 'causalforest' and CausalForestDML is not None:
            ...
            if hasattr(self, 'causal_forest_model') and self.causal_forest_model is not None:
                 ...
                 print(f"Estimated average sales if ALL treated: {avg_sales_if_all_treated:.2f}")
                 print(f"Estimated average sales if NONE treated: {avg_sales_if_none_treated:.2f}")
                 return {"all_treated": avg_sales_if_all_treated, "none_treated": avg_sales_if_none_treated}
            else:
                 print("CausalForest model not trained or available for counterfactuals.")
                 return None
        else:
            print(f"Counterfactual simulation for model_type '{model_type}' not implemented or library missing.")
            return None
    ...
    def visualize_causal_graph(self, save_path: Optional[str] = None):
        ...
        if not hasattr(self, 'causal_graph') or not isinstance(self.causal_graph, nx.DiGraph):
    ...
    def regression_adjustment(self) -> Dict[str, float]:
        ...
        try:
            ...
            feature_cols = [col for col in potential_features if pd.api.types.is_numeric_dtype(self.analysis_data[col])]
            categorical_cols = [c for c in self.common_causes if c != 'holiday' and c not in feature_cols]
            
            analysis_subset = self.analysis_data[feature_cols + categorical_cols + [self.treatment, self.outcome]].copy()

            if categorical_cols:
                analysis_subset = pd.get_dummies(analysis_subset, columns=categorical_cols, drop_first=True)
                feature_cols = [col for col in analysis_subset.columns if col not in [self.outcome, self.treatment]] + [self.treatment]
            ...
            X.columns = ["_".join(x.split()) if isinstance(x, tuple) else "".join (c if c.isalnum() else "_" for c in str(x)) for x in X.columns]
            ...
            if treatment_col_clean not in model.params.index:
                 return {'error': f'Treatment variable "{treatment_col_clean}" not found in model results after cleaning names.'}
            ...
            if isinstance(conf_int_result, pd.DataFrame):
                ...
                print(f"Warning: Unexpected type for confidence interval: {type(conf_int_result)}")
            ...
            percent_lift = ((promotion_sales_pred.mean() / baseline_mean - 1) * 100
    ...
    def matching_analysis(self, caliper: float = 0.05, ratio: int = 1) -> Dict[str, float]:
        """Estimate promotion impact using propensity score matching"""
        from sklearn.linear_model import LogisticRegression # Ensure sklearn is imported or available
        import numpy as np # Ensure numpy is imported or available
        ...
        try:
            ...
            if categorical_cols:
                ...
                feature_cols = [col for col in analysis_subset.columns if col not in [self.outcome, self.treatment]]
            ...
            for treat_idx, treat_row in treatment_group.iterrows():
                ...
                potential_matches = control_group.drop(index=list(used_control_indices), errors='ignore').copy()
                if potential_matches.empty: continue

                potential_matches['distance'] = abs(potential_matches['propensity_score'] - treat_row['propensity_score'])

                # Filter by caliper
                within_caliper = potential_matches[potential_matches['distance'] <= caliper]
                if within_caliper.empty: continue
                ...
                for control_idx, control_row in closest_matches.iterrows():
                    if control_idx not in used_control_indices:
                        ...
                        used_control_indices.add(control_idx) # Mark control as used
    ...
    def double_ml_forest(self, **kwargs) -> Dict[str, Union[float, Dict[str, float]]]:
        ...
        return {"skipped": True, "reason": "DoubleML Forest implementation requires specific feature/modifier setup"}


    def dowhy_analysis(self, **kwargs) -> Dict[str, float]:
        ...
        return {"skipped": True, "reason": "DoWhy analysis implementation requires graph/model setup"}
    ...
    def perform_counterfactual_analysis(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            feature_cols_model = [col for col in potential_features_model if pd.api.types.is_numeric_dtype(self.analysis_data[col])]
            categorical_cols_model = [c for c in self.common_causes if c != 'holiday' and c not in feature_cols_model]
            analysis_subset_model = self.analysis_data[feature_cols_model + categorical_cols_model + [self.treatment, self.outcome]].copy()

            if categorical_cols_model:
                analysis_subset_model = pd.get_dummies(analysis_subset_model, columns=categorical_cols_model, drop_first=True)
                feature_cols_model = [col for col in analysis_subset_model.columns if col not in [self.outcome, self.treatment]] + [self.treatment]
            ...
            X_actual.columns = ["_".join(x.split()) if isinstance(x, tuple) else "".join (c if c.isalnum() else "_" for c in str(x)) for x in X_actual.columns]
            
            model = sm.OLS(Y_actual, X_actual).fit()
            # <<< Get the exact column names used by the fitted model >>>
            ...
            X_cf.columns = ["_".join(x.split()) if isinstance(x, tuple) else "".join (c if c.isalnum() else "_" for c in str(x)) for x in X_cf.columns]
    ...
    def calculate_promotion_roi(self, promotion_cost_per_instance: float, margin_percent: float) -> Dict[str, float]:
        ...
        try:
            ...
            if 'error' in regression_results:
                return {'error': f"Cannot calculate ROI due to regression error: {regression_results['error']}"}
            ...
            if self.analysis_data[self.treatment].isnull().any():
                print(f"--- WARNING ROI: NaNs found in treatment column '{self.treatment}'. Check data preparation.")
            ...
            roi_per_instance = ((incremental_margin_per_instance - promotion_cost_per_instance) / promotion_cost_per_instance) * 100 \
                               if promotion_cost_per_instance != 0 else float('inf')
            ...
            if not isinstance(num_promotion_instances, (int, np.integer)):
                 print(f"--- WARNING ROI: num_promotion_instances is not scalar int ({type(num_promotion_instances)}). Attempting conversion.")
                 try:
                     num_promotion_instances = int(num_promotion_instances)
                 except Exception as conv_e:
                     print(f"--- ERROR ROI: Failed to convert num_promotion_instances to int: {conv_e}")
            ...
            total_roi_est = ((total_incremental_margin_est - total_promotion_cost_est) / total_promotion_cost_est) * 100 \
                             if total_promotion_cost_est != 0 else float('inf')
