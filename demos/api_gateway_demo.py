...
async def inventory_proxy(request: Request, path: str, background_tasks: BackgroundTasks, agent: Agent = Depends(get_current_active_agent)):
...
async def product_proxy(request: Request, path: str, background_tasks: BackgroundTasks, agent: Agent = Depends(get_current_active_agent)):
