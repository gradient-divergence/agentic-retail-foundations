...
from models.enums import InventoryEventType # Use the centralized enum
from models.state import ProductInventoryState # Use the centralized state model
...
event_redis: Optional[redis.Redis] = None
state_redis: Optional[redis.Redis] = None
...
async def shutdown_event():
    if event_redis: await event_redis.close()
    if state_redis: await state_redis.close()
...
async def get_crdt_state(product_id: str, location_id: str) -> PNCounter:
    ...
    if not state_redis: raise HTTPException(503, "State DB not available")
