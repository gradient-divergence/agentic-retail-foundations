...
from models.supplier import Supplier, SupplierRating, SupplierStatus
...
class PurchaseOrder:
    ...
    winning_bid_details: Optional[SupplierBid] = None # Store the winning bid object
...
class ProcurementAuction:
    ...
    def register_supplier(self, supplier: Supplier) -> None:
        ...
        print(f"Supplier {supplier.name} ({supplier.supplier_id}) registered for auctions.")
    ...
    def create_purchase_order(
        ...
        self.bids[po.id] = [] 
        print(f"Created Purchase Order {po.id} for {quantity}x{product_id}, budget ${budget}, deadline {deadline_days} days.")
        return po.id

    async def collect_bids(
        self, po_id: str, bid_window_seconds: float = 0.1 # Reduced default window
    ) -> List[SupplierBid]:
        """
        ...
        if purchase_order.status != PurchaseOrderStatus.BIDDING:
             print(f"Warning: PO {po_id} is not in BIDDING state (current: {purchase_order.status.name}). Cannot collect new bids.")
             return self.bids.get(po_id, []) # Return existing bids if any

        print(f"--- Collecting bids for PO {po_id} ({purchase_order.quantity}x{purchase_order.product_id}) --- ")
        ...
        print(f"--- Bid collection finished for PO {po_id}. Total bids: {len(self.bids[po_id])} ---")
        return self.bids[po_id]

    def _simulate_supplier_bid_calculation(self, supplier: Supplier, purchase_order: PurchaseOrder) -> Optional[SupplierBid]:
         ...
         # Volume discounts
         if purchase_order.quantity > 1000: total_price *= 0.90
         elif purchase_order.quantity > 500: total_price *= 0.95
         ...
         quality_guarantee = min(0.99, 0.80 + (supplier.rating.value * 0.06) * (1.0 / max(0.1, supplier.quality_factor))) 
    ...
    def evaluate_bids(self, po_id: str) -> Optional[str]:
        ...
        winning_bid: Optional[SupplierBid] = None
        ...
        for bid in bids_to_evaluate:
            ...
            days_allowed = max(1, (purchase_order.required_delivery_date - bid.timestamp.replace(tzinfo=None)).days)
            ...
            print(f"  Bidder: {self.suppliers[bid.supplier_id].name}, Score: {score:.4f} (P:{price_norm:.2f}, D:{delivery_norm:.2f}, Q:{quality_norm:.2f})")
