...
def product_info() -> ProductInfo:
    return ProductInfo(
        product_id="P101", name="Test Coffee", category="Beverage",
        price=12.0, cost=6.0, supplier_id="SUP_A", lead_time_days=3,
        shelf_life_days=30, min_order_quantity=10
    )

@pytest.fixture
def inventory_item() -> InventoryItem:
    return InventoryItem(
        product_id="P101", current_stock=50, reorder_point=20, optimal_stock=60,
        last_reorder_date=datetime(2024, 1, 1), pending_order_quantity=0,
...
def test_observe_success(mock_fetch_lead, mock_fetch_comp, bdi_agent: InventoryBDIAgent, product_info, inventory_item, sales_data):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "optimal_normal", 50, [4,5,6,4,5,6,5], 12.0, 3,
            {
                "price_position": "competitive", "inventory_status": "optimal",
                "sales_assessment": "normal", "market_situation": "balanced",
                "days_of_supply": pytest.approx(50/5.0), "projected_daily_sales": 5.0
            }
        ),
        # Case 2: Low Stock, High Sales -> Risk of Stockout
        (
            "low_stock_high_sales", 15, [8,9,10,8,9,10,9], 12.0, 3, # Inv < RP=20, DoS=15/9=1.6 < LT+3=6
            {
                "price_position": "competitive", "inventory_status": "low",
                "sales_assessment": "risk_of_stockout", "market_situation": "high_demand_low_supply",
                "days_of_supply": pytest.approx(15/9.0), "projected_daily_sales": 9.0
            }
        ),
        # Case 3: High Stock, Low Sales -> Slow Moving
        (
            "high_stock_low_sales", 100, [0,1,0,0,1,0,0], 12.0, 3, # Inv > OS*1.5=90, Sales < RP*0.1=2
            {
                "price_position": "competitive", "inventory_status": "high",
                "sales_assessment": "slow_moving", "market_situation": "low_demand_high_supply",
                "days_of_supply": pytest.approx(100/0.29, 0.1), "projected_daily_sales": pytest.approx(0.29, 0.1)
            }
        ),
        ...
        (
            "discount_normal", 50, [4,5,6,4,5,6,5], 14.0, 3, # Price=12 < 14*0.9
            {
                "price_position": "discount", "inventory_status": "optimal",
                "sales_assessment": "normal", "market_situation": "underpriced",
                "days_of_supply": pytest.approx(50/5.0), "projected_daily_sales": 5.0
            }
        ),
    ],
...
def test_orient(
    bdi_agent: InventoryBDIAgent, product_info: ProductInfo, inventory_item: InventoryItem,
    test_id: str, inventory_level: int, sales_7d: list[int], avg_comp_price: float, lead_time: int,
    ...
    observation = {
        ...
        "competitor_prices": {"CompX": avg_comp_price - 0.5, "CompY": avg_comp_price + 0.5},
        "lead_time": lead_time,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ["minimize_stockouts", "ensure_fresh_products", "minimize_excess_inventory", "maximize_profit_margin"]
        ...
        (
            ...
            ["minimize_excess_inventory", "minimize_stockouts", "ensure_fresh_products", "maximize_profit_margin"]
        ...
        (
            ...
            ["maximize_profit_margin", "minimize_stockouts", "ensure_fresh_products", "minimize_excess_inventory"]
        ...
        (
            ...
            ["ensure_fresh_products", "minimize_stockouts", "minimize_excess_inventory", "maximize_profit_margin"]
...
def test_deliberate(
    mock_eval_fresh, mock_eval_profit, mock_eval_excess, mock_eval_stockout,
    bdi_agent: InventoryBDIAgent,
    test_id: str, mock_utilities: dict, expected_goal_order: list[str]
):
...
def test_generate_intentions_calls_correct_planners(
    mock_plan_fresh, mock_plan_margin, mock_plan_excess, mock_plan_reorder,
    bdi_agent: InventoryBDIAgent
):
    ...
    mock_plan_reorder.reset_mock(); mock_plan_excess.reset_mock()
    mock_plan_margin.reset_mock(); mock_plan_fresh.reset_mock()
...
def test_execute_intentions_prioritization_and_filtering(
    mock_exec_perish, mock_exec_promo, mock_exec_discount, mock_exec_reorder,
    bdi_agent: InventoryBDIAgent
):
    ...
    intention_reorder_p1_high = {"action": "reorder", "product_id": "P1", "priority": 0.9}
    intention_discount_p1_low = {"action": "discount", "product_id": "P1", "priority": 0.3}
    intention_promote_p2 = {"action": "promote", "product_id": "P2", "priority": 0.7}
    intention_perish_p3 = {"action": "discount_perishable", "product_id": "P3", "priority": 0.5}
...
def test_execute_intentions_handles_execution_error(
    mock_exec_reorder,
    bdi_agent: InventoryBDIAgent,
    caplog
):
    ...
    with patch.object(InventoryBDIAgent, '_execute_promotion', return_value=True) as mock_exec_promo, \
