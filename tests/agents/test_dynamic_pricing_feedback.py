...
def test_agent_initialization_success(
    mock_kafka_consumer_cls, mock_kafka_producer_cls, mock_redis_cls,
    agent_params
):
    ...
    mock_redis_cls.assert_called_once_with(
        host=agent_params["redis_host"], port=agent_params["redis_port"], decode_responses=True
    )
...
async def test_get_recent_sales_no_client(agent_params):
    ...
    with patch('redis.asyncio.Redis', side_effect=ConnectionError("Init fail")), \
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "hits_min_price", 41.0, -1.5,
            [10], [(1, 15)], # Demand increases
            40.0, 70.0,
        ...
        (
            "hits_max_price", 69.0, -1.5,
            [10], [(1, 5)], # Demand decreases
            40.0, 70.0,
...
def test_compute_optimal_price(
    ...
    test_id: str, current_price: float, price_elasticity: float,
    demand_history: list[float], recent_sales_list: list[tuple[int, float]],
    min_price: float, max_price: float, expected_price: float
):
...
async def test_update_price_success(mock_dt, mocked_agent: DynamicPricingAgent):
    ...
    mock_redis.execute_command.assert_awaited_once_with(
        "TS.ADD", expected_redis_key, "*", str(new_price)
    )
...
async def test_process_sales_feedback_success(
    ...
    mock_consumer.poll.return_value = {
        MagicMock(): [mock_record1, mock_record_other, mock_record2]
    }
...
@pytest.mark.asyncio
@patch.object(DynamicPricingAgent, 'update_elasticity_model', new_callable=AsyncMock)
async def test_process_sales_feedback_no_messages(
    mock_update_elasticity, mocked_agent: DynamicPricingAgent
):
...
@pytest.mark.asyncio
@patch.object(DynamicPricingAgent, 'update_elasticity_model', new_callable=AsyncMock)
async def test_process_sales_feedback_kafka_error(
...
async def test_process_sales_feedback_no_consumer(agent_params):
    ...
    with patch('redis.asyncio.Redis'), patch('kafka.KafkaProducer'), \
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "normal_update", 50.0, 45.0, # Price decreased 10%
            10.0, 12.0, # Demand increased 20%
            -1.5, 0.1, # Initial elasticity, LR
        ...
        (
            "positive_elasticity_skip", 50.0, 45.0, # Price decreased
            10.0, 9.0,  # Demand also decreased (positive elasticity)
            -1.5, 0.1,
            -1.5, False # Elasticity unchanged
        ),
        ...
        (
            "bounded_lower", 50.0, 49.0, # Price decreased 2%
        ...
        (
            "bounded_upper", 50.0, 49.0, # Price decreased 2%
...
async def test_update_elasticity_model(
    mocked_agent: DynamicPricingAgent,
    test_id: str, p1: float, p2: float, d1: float, d2: float,
    initial_elasticity: float, learning_rate: float,
    expected_new_elasticity: float, expect_redis_set: bool,
    caplog
):
