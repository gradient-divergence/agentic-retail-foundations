...
@pytest.mark.asyncio
async def test_handle_order_allocated_missing_order_id(
    fulfillment_agent: FulfillmentAgent, mock_event_bus: AsyncMock, caplog
):
...
async def test_handle_order_allocated_publish_error(
    mock_handle_exception, mock_publish_event,
    fulfillment_agent: FulfillmentAgent, mock_event_bus: AsyncMock, caplog
):
    ...
    assert exc_call_args["context"] == {"stage": "payment_initiation", "order_id": order_id}
...
from models.enums import FulfillmentMethod
...
def test_group_items_by_fulfillment_mixed(fulfillment_agent: FulfillmentAgent):
    ...
    item1 = OrderLineItem(product_id="P1", quantity=1, price=10.0, fulfillment_method=FulfillmentMethod.SHIP_FROM_STORE, fulfillment_location_id="S1")
    item2 = OrderLineItem(product_id="P2", quantity=2, price=5.0, fulfillment_method=FulfillmentMethod.SHIP_FROM_STORE, fulfillment_location_id="S1")
    item3 = OrderLineItem(product_id="P3", quantity=1, price=20.0, fulfillment_method=FulfillmentMethod.SHIP_FROM_WAREHOUSE, fulfillment_location_id="W1")
    item4 = OrderLineItem(product_id="P4", quantity=1, price=15.0, fulfillment_method=FulfillmentMethod.PICKUP_IN_STORE, fulfillment_location_id="S2")
    item5_missing = OrderLineItem(product_id="P5", quantity=1, price=1.0) # Missing details, still needs price

    order = Order(
        order_id="GRP_TEST_1", customer_id="C1", items=[item1, item2, item3, item4, item5_missing],
        status=OrderStatus.CREATED # Use correct enum
    )

    groups = fulfillment_agent._group_items_by_fulfillment(order)

    # Expected groups: (SHIP_FROM_STORE, S1), (SHIP_FROM_WAREHOUSE, W1), (PICKUP_IN_STORE, S2)
    assert len(groups) == 3
...
def test_group_items_by_fulfillment_single_group(fulfillment_agent: FulfillmentAgent):
    ...
    item1 = OrderLineItem(product_id="P1", quantity=1, price=10.0, fulfillment_method=FulfillmentMethod.PICKUP_IN_STORE, fulfillment_location_id="S1")
    item2 = OrderLineItem(product_id="P2", quantity=1, price=12.0, fulfillment_method=FulfillmentMethod.PICKUP_IN_STORE, fulfillment_location_id="S1")
    order = Order(order_id="GRP_TEST_2", customer_id="C2", items=[item1, item2], status=OrderStatus.CREATED)
...
def test_group_items_by_fulfillment_invalid_item(fulfillment_agent: FulfillmentAgent, caplog):
    ...
    item_valid = OrderLineItem(product_id="P1", quantity=1, price=10.0, fulfillment_method=FulfillmentMethod.SHIP_FROM_STORE, fulfillment_location_id="S1")
    item_invalid = OrderLineItem(product_id="P_INVALID", quantity=1, price=1.0) # Missing details, still needs price
    order = Order(order_id="GRP_TEST_4", customer_id="C4", items=[item_valid, item_invalid], status=OrderStatus.CREATED)
    ...
    assert f"Item P_INVALID in order GRP_TEST_4 missing fulfillment details." in caplog.text
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("delivery_from_store", FulfillmentMethod.DELIVERY_FROM_STORE, AgentType.STORE_OPS),
        ("ship_from_warehouse", FulfillmentMethod.SHIP_FROM_WAREHOUSE, AgentType.WAREHOUSE),
    ],
    ids=lambda x: x[0] if isinstance(x, str) else "" # Use test_id
)
@patch.object(FulfillmentAgent, 'publish_event', new_callable=AsyncMock)
async def test_initiate_fulfillment_request_success(
    mock_publish, fulfillment_agent: FulfillmentAgent,
    test_id: str, method: FulfillmentMethod, expected_target_agent: AgentType
):
    """Test initiating fulfillment request for different methods."""
    item1 = OrderLineItem(product_id="P1", quantity=1, price=10.0)
    order = Order(order_id="INIT_TEST_1", customer_id="C1", items=[item1], status=OrderStatus.CREATED)
...
@pytest.mark.asyncio
@patch.object(FulfillmentAgent, 'publish_event', new_callable=AsyncMock)
async def test_initiate_fulfillment_request_unknown_method(
    mock_publish, fulfillment_agent: FulfillmentAgent, caplog
):
    """Test initiating fulfillment with an unknown method logs error."""
    item1 = OrderLineItem(product_id="P1", quantity=1, price=10.0)
    order = Order(order_id="INIT_TEST_2", customer_id="C1", items=[item1], status=OrderStatus.CREATED)
    location = "LOC_X"
    items_list = [item1]
...
async def test_handle_payment_processed_success(
    mock_publish, mock_initiate_request, mock_group_items, mock_get_order,
    fulfillment_agent: FulfillmentAgent, mock_event_bus: AsyncMock
):
...
async def test_handle_payment_processed_order_not_found(
    mock_publish, mock_initiate_request, mock_group_items, mock_get_order,
    fulfillment_agent: FulfillmentAgent, caplog
):
