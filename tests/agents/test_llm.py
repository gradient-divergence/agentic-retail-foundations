...
from agents.conversation_manager import ConversationManager # For type hinting if needed
from agents.llm_context_retriever import LLMContextRetriever # For type hinting if needed
from agents.llm_components import build_response_prompt, generate_agent_response, extract_actions
...
async def test_process_customer_inquiry_order_status(
    ...
    mock_connectors[1].get_recent_orders = AsyncMock(return_value=[
    ...
    # So, ensuring mock_connectors[1] (order_system mock) has get_order_details is good if get_context is complex.
    # For this test, mocking get_context directly is simpler.
    agent.context_retriever.get_context = AsyncMock(return_value={
        "order_details": mock_order_details
    })
    ...
    agent.context_retriever.get_context.assert_awaited_once_with(
        "order_status", {"order_id": test_order_id}
    )
    ...
    mock_nlp_sentiment.assert_awaited_once_with(client=agent.client, message=message, model=agent.utility_model, logger=agent.logger, retry_attempts=agent.retry_attempts, retry_backoff=agent.retry_backoff)
    ...
    # No, extract_order_id is called directly by agent, not via the mock_nlp_module fixture here
    # mock_nlp_module.extract_order_id_llm.assert_awaited_once() # This would be for the fixture
