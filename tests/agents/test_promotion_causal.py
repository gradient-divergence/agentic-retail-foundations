...
def test_analyzer_initialization_success(
    ...
    # Access the keyword arguments from the call
...
def test_analyzer_initialization_data_prep_fails(
    ...
    mock_define_graph.assert_not_called() # Graph definition should be skipped
...
def test_visualize_graph_no_graph_defined(
    ...
    """Test visualize_graph does nothing and prints if no graph is defined."""
    mock_prepare_data.return_value = mock_prepared_data
    mock_define_graph.side_effect = ValueError("Graph def error") 
    analyzer = PromotionCausalAnalyzer(sales_data=sample_sales_data)
    ...
    assert "Error: Causal graph is not available" in captured.out
...
def test_visualize_graph_visualization_error(
    ...
    """Test visualize_graph handles errors from the visualization function via print."""
    mock_prepare_data.return_value = mock_prepared_data
    mock_define_graph.return_value = mock_graph_definition
    ...
    captured = capsys.readouterr()
    assert "Error during graph visualization call: Plotting failed" in captured.out
...
def test_run_all_analyses_no_analysis_data(
    ...
    captured = capsys.readouterr() 
    assert "Error: Analysis data not properly initialized" in captured.out
...
def test_run_all_analyses_default_estimators_success(
    ...
    captured = capsys.readouterr()
    assert "Running Naive Estimator..." in captured.out
    assert "Running Regression Adjustment..." in captured.out
    assert "Running Propensity Score Matching..." in captured.out
    assert "Running Causal Forest DML..." in captured.out
...
def test_run_all_analyses_custom_estimators_success(
    ...
    captured = capsys.readouterr()
    assert "Running DoWhy (Linear Regression)..." in captured.out
    assert "Running DoWhy (Propensity Score Matching)..." in captured.out
    assert "Running DoubleML IRM (Random Forest)..." in captured.out
    assert "Running DoubleML IRM (Lasso)..." in captured.out
...
def test_run_all_analyses_missing_common_causes(
    ...
    captured = capsys.readouterr()
    assert "Skipping Regression Adjustment (no common causes)." in captured.out
    assert "Skipping Propensity Score Matching (no common causes)." in captured.out
    assert "Skipping Causal Forest DML (no common causes)." in captured.out
...
def test_run_all_analyses_missing_graph(
    ...
    captured = capsys.readouterr()
    assert "Skipping DoWhy (Linear Regression) (no causal graph)." in captured.out
    assert "Skipping DoWhy (Propensity Score Matching) (no causal graph)." in captured.out
...
def test_run_all_analyses_estimator_exception(
    ...
    captured = capsys.readouterr()
    assert "ERROR running regression estimator: Regression failed spectacularly" in captured.out
    assert "Running Naive Estimator..." in captured.out
    assert "Running Propensity Score Matching..." in captured.out
    assert "Running Causal Forest DML..." in captured.out
...
def test_fit_model_for_counterfactuals_success(
    ...
    captured = capsys.readouterr()
    assert "Model 'causal_forest' fitted and stored." in captured.out
...
def test_fit_model_for_counterfactuals_no_analysis_data(
    ...
    captured = capsys.readouterr()
    assert "Error: Analysis data not initialized. Cannot fit model." in captured.out
...
def test_fit_model_no_common_causes_for_causal_forest(
    ...
    captured = capsys.readouterr()
    assert "Error: Common causes not available. Cannot fit 'causal_forest' model." in captured.out
...
def test_fit_model_fitter_exception(
    ...
    captured = capsys.readouterr()
    assert "ERROR fitting causal_forest model: CF fitting exploded" in captured.out
...
def test_fit_model_unsupported_key(
    ...
    captured = capsys.readouterr()
    assert "Model type 'super_xgb_tree' not currently supported" in captured.out
...
def test_run_counterfactual_analysis_success(
    ...
    captured = capsys.readouterr()
    assert "Counterfactual Simulation Results:" in captured.out
    assert "Ate Lift: 50.0000" in captured.out 
...
def test_run_counterfactual_analysis_no_analysis_data(
    ...
    captured = capsys.readouterr()
    assert "Error: Analysis data not initialized. Cannot run counterfactuals." in captured.out
...
def test_run_counterfactual_analysis_model_not_fitted(
    ...
    assert results is None
    captured = capsys.readouterr()
    assert "Error: Model 'my_missing_model' not found or not fitted." in captured.out
...
def test_run_counterfactual_analysis_simulation_error(
    ...
    captured = capsys.readouterr()
    assert "ERROR during counterfactual simulation call: Simulation exploded" in captured.out
...
def test_run_counterfactual_analysis_no_common_causes(
    ...
    captured = capsys.readouterr()
    assert "Error: Common causes not available, cannot run counterfactuals for 'causal_forest'" in captured.out
...
def test_run_roi_analysis_success_from_ate_source(
    ...
    captured = capsys.readouterr()
    assert "Using ATE=0.5000 from 'causal_forest' for ROI calculation." in captured.out
    assert "ROI Interpretation: Promotion was very profitable." in captured.out
...
def test_run_roi_analysis_success_direct_ate(
    ...
    captured = capsys.readouterr()
    assert "ROI Interpretation: Direct ATE showed good profit." in captured.out
...
def test_run_roi_analysis_no_analysis_data(
    ...
    mock_prepare_data.side_effect = ValueError("Data prep error")
    # Setup analyzer with data prep failure
    analyzer = PromotionCausalAnalyzer(sales_data=sample_sales_data)
    assert analyzer.analysis_data is None

    results = analyzer.run_roi_analysis(estimated_ate=0.5) # Try to run ROI anyway
    assert results is None
    captured = capsys.readouterr()
    # Check for the print within _initialize_data_and_graph first
    assert "ERROR during data preparation: Data prep error" in captured.out
    # Then check for the print within run_roi_analysis
    assert "Error: Analysis data not available for ROI calculation." in captured.out
...
def test_run_roi_analysis_ate_source_not_found(
    ...
    assert results is None
    captured = capsys.readouterr()
    assert "Error: Result for ATE source 'non_existent_method' not found" in captured.out
...
def test_run_roi_analysis_ate_extraction_error(
    ...
    captured_wrong_key = capsys.readouterr()
    assert "Error: Could not extract a valid numeric ATE value" in captured_wrong_key.out

    # Scenario 2: Result for source was an error dictionary
    ...
    captured_prev_error = capsys.readouterr()
    assert "Error: Cannot use ATE from 'error_method' due to previous error: previous estimation failure" in captured_prev_error.out
...
def test_run_roi_analysis_stat_calc_key_error(
    ...
    mock_prepare_data.return_value = mock_prepared_data.copy() 
    mock_define_graph.return_value = mock_graph_definition
    # Initialize analyzer with known treatment and outcome names
    ...
    # Scenario 1: Treatment column missing from analysis_data
    analysis_data_no_treatment = analyzer.analysis_data.copy()
    if analyzer.treatment in analysis_data_no_treatment.columns:
        analysis_data_no_treatment = analysis_data_no_treatment.drop(columns=[analyzer.treatment])
    analyzer.analysis_data = analysis_data_no_treatment # Assign modified data
    ...
    analyzer.analysis_data[analyzer.treatment] = [0,0,1,1] # Ensure treatment exists for next check

    # Scenario 2: Outcome column missing
    analysis_data_no_outcome = analyzer.analysis_data.copy()
    if analyzer.outcome in analysis_data_no_outcome.columns:
        analysis_data_no_outcome = analysis_data_no_outcome.drop(columns=[analyzer.outcome])
    analyzer.analysis_data = analysis_data_no_outcome # Assign modified data
...
def test_run_roi_analysis_interpretation_exception(
    ...
    captured = capsys.readouterr()
    assert "ERROR during ROI calculation/interpretation call: Interpret error" in captured.out
...
def test_run_roi_analysis_calculation_exception(
    ...
    captured = capsys.readouterr()
    assert "ERROR during ROI calculation/interpretation call: Calc ROI error" in captured.out

# --- Tests for _is_holiday (example helper) ---

# Test for _is_holiday would require making it accessible or testing via a public method that uses it.
# For now, if it's only used internally by prepare_analysis_data, its effects might be covered there.
# If direct testing is desired, consider making it a static method or a free utility function.

def test_is_holiday_helper(sample_sales_data, capsys): 
    analyzer = PromotionCausalAnalyzer(sales_data=sample_sales_data) 
    ...
    expected_holidays = pd.Series([True, False, True, False, True, False]) # 2024-01-01 is not in fixed list
    ...
    captured = capsys.readouterr()
    assert "Warning: Failed to check holidays" in captured.out
