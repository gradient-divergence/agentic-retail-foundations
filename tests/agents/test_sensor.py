...
async def test_process_sensor_message_recent_readings(mock_dt, sensor_processor: SensorDataProcessor):
    ...
    with patch.object(SensorDataProcessor, "_process_rfid_reading") as _mock_process_rfid:
        await sensor_processor.process_sensor_message(msg_old)
        # Pruning happens here based on msg_old's timestamp vs fixed_now
        # -> msg_old IS older than 24h
        # but recent_readings[sensor_id] is created *inside* this call, so pruning
        # an empty list? No, it's added first.
        # Let's trace: process(msg_old) -> recent_readings={'s1': [msg_old]}
        # -> prune(cutoff=24h ago) -> recent_readings={'s1': []} ?
        # No, prune compares item ts > cutoff
        # item ts = 2d ago. cutoff = now - 24h. So msg_old ts is NOT > cutoff.
        # It should be removed.
        # So after first call, recent_readings[s1] should be empty.

        await sensor_processor.process_sensor_message(msg1)
        # msg1 ts = 1h ago. cutoff = now - 24h. msg1 ts > cutoff. Should be kept.
        # recent_readings={'s1': [msg1]}
...
async def test_process_rfid_reading_success_match(
    ...
    mock_inventory_system.get_expected_products.assert_awaited_once_with(sensor_processor.store_id, expected_location_str)
    ...
    expected_update_payload = [
        {
            "product_id": "P1",
            "location": expected_location_str,
            "last_seen": timestamp,
            "confidence": 0.9,
        },
        {
...
async def test_process_rfid_reading_missing_and_unexpected(
    ...
    message = {
        ...
        "location": location,
        "confidence": 0.9,
        "detected_products": detected_list,
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            4.0,
            "C",
            {"zone": "Ref", "section": "1", "zone_type": "refrigerated"},
            False,
            None,
            False,
        ),
        ...
        (
            ...
            -18.0,
            "C",
            {"zone": "Frz", "section": "A", "zone_type": "frozen"},
            False,
            None,
            False,
        ),
        ...
        (
            ...
            22.0,
            "C",
            {"zone": "Amb", "section": "5", "zone_type": "ambient"},
            False,
            None,
            False,
        ),
        ...
        (
            ...
            85.0,
            "%",
            {"zone": "Prod", "section": "P1", "zone_type": "produce"},
            False,
            None,
            False,
        ),
...
async def test_process_environmental_reading(
    ...
    timestamp = datetime.now().isoformat()
    message = {
        "sensor_id": f"env_{test_id}",
        "sensor_type": "environmental",
        "timestamp": timestamp,
        "environmental_type": sensor_type,
        "value": value,
        "unit": unit,
        "location": location,
    }
...
async def test_process_price_tag_reading_low_battery(
    ...
    message = {
        "sensor_id": "tag_sensor_1",
        "sensor_type": "digital_price_tag",
        "timestamp": datetime.now().isoformat(),
        "tag_id": tag_id,
        "product_id": product_id,
        "location": location,
        "price_displayed": 19.99,
        "battery_level": 15,  # Low battery
    }

    # Mock inventory system to return a price (doesn't matter if it matches for this
    # test)
...
async def test_process_price_tag_reading_price_mismatch(
    ...
    message = {
        "sensor_id": "tag_sensor_2",
        "sensor_type": "digital_price_tag",
        "timestamp": datetime.now().isoformat(),
        "tag_id": tag_id,
        "product_id": product_id,
        "location": location,
        "price_displayed": displayed_price,
        "battery_level": 90,  # Battery OK
    }
...
async def test_handle_inventory_discrepancy_first_detection(
    ...
    """Test first detection of a discrepancy creates a record but doesn't alert/report
    yet."""
    location = "A.1"
    product_id = "P1"
    ...
    assert _record["location"] == location
    assert _record["type"] == discrepancy_type
    assert _record["detection_count"] == 1
    assert _record["sources"] == [source]
    assert _record["details"] == details
    assert "first_detected" in _record
    assert "last_updated" in _record

    # Verify confidence was calculated
    mock_calc_conf.assert_called_once_with(_record)
...
async def test_handle_inventory_discrepancy_subsequent_detection(
    ...
    # First detection
    await sensor_processor._handle_inventory_discrepancy(location, [product_id], discrepancy_type, source1, details1)
    discrepancy_key = f"{product_id}:{location}:{discrepancy_type}"
    ...
    await sensor_processor._handle_inventory_discrepancy(location, [product_id], discrepancy_type, source2, details2)
...
async def test_handle_inventory_discrepancy_reaches_confidence_threshold(
    ...
    mock_inventory_system.report_inventory_issue.assert_awaited_once_with(
        sensor_processor.store_id, product_id, location, discrepancy_type, details, 0.95
    )
...
def test_calculate_discrepancy_confidence(sensor_processor: SensorDataProcessor):
    ...
    # Count 5 -> + 0.2
    record3 = {
        "sources": ["rfid", "smart_shelf", "computer_vision"],
        "detection_count": 5,
    }
