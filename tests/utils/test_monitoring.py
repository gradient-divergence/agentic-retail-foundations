...
from utils.monitoring import AgentMonitor, PerformanceMonitor
...
def _mock_check_drift_helper(metric_name, value, test_id, adaptation_monitor):
    if metric_name == "error_rate":
        if "error_rate_drift_increasing" in test_id or "multiple_drifts_recos" in test_id:
            _, max_thresh = adaptation_monitor.metric_thresholds[metric_name]
            return value > max_thresh
    return False
...
def test_performance_monitor_check_metrics_alerts(
    ...
    alert_endpoints = [
        {"type": "slack", "webhook_url": "mock_url", "handler": mock_alert_slack},
        {"type": "email", "address": "test@example.com", "handler": mock_alert_email},
    ]
    monitor = PerformanceMonitor(agent_id, thresholds, alert_endpoints, window_size=1)

    caplog.clear()
    with caplog.at_level(logging.WARNING):  # Alerts are logged as WARNING or ERROR
        monitor.log_metric(metric_name, metric_value)

    assert mock_alert_slack.call_count == expected_alerts
    assert mock_alert_email.call_count == expected_alerts
    ...
    if expected_alerts > 0:
        ...
        # Check that the mock handlers were called with the alert message
        mock_alert_slack.assert_any_call(alert_message)
        mock_alert_email.assert_any_call(alert_message)
    else:
...
def adaptation_monitor(mock_alert_slack):  # Using one alert mock for simplicity
    agent = MockAgent(agent_id="adapt_agent_01")
    thresholds = {
    ...
    alert_endpoints = [{"type": "slack", "webhook_url": "mock_url", "handler": mock_alert_slack}]
    return PerformanceMonitor(agent.agent_id, thresholds, alert_endpoints, window_size=5, agent_instance=agent)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ["Investigate source of increasing error_rate."],
            True,
        ),
        (
...
@patch("utils.monitoring.DataDriftDetector.check_drift")
@patch.object(PerformanceMonitor, "_is_decreasing")  # Patching the method on the class
def test_recommend_adaptation_scenarios(
    mock_is_decreasing: MagicMock,
    mock_check_drift: MagicMock,
    adaptation_monitor: PerformanceMonitor,
    test_id: str,
    ...
    mock_check_drift.side_effect = lambda m, v: _mock_check_drift_helper(m, v, test_id, adaptation_monitor)

    if adaptation_monitor.agent_instance:
        adaptation_monitor.agent_instance.logger = logging.getLogger("MockAgentLogger")  # Give mock agent a logger for caplog
    ...
    if expect_adaptation_log and adaptation_monitor.agent_instance:
    ...
    elif adaptation_monitor.agent_instance:
