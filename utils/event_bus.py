...
class EventBus:
    ...
    def __init__(self):
        self.subscribers: dict[
            str, list[Callable[[RetailEvent], Coroutine[Any, Any, None]]]
        ] = {}
    ...
    def subscribe(
        ...
        if (
            callback not in self.subscribers[event_type]
        ):  # Avoid duplicate subscriptions
            self.subscribers[event_type].append(callback)
            logger_event_bus.debug(
                f"Callback {callback.__name__} subscribed to {event_type}"
            )
        else:
            logger_event_bus.warning(
                f"Callback {callback.__name__} already subscribed to {event_type}"
            )
    ...
    def unsubscribe(
        ...
        if event_type in self.subscribers:
            try:
                ...
                logger_event_bus.debug(
                    f"Callback {callback.__name__} unsubscribed from {event_type}"
                )
                if not self.subscribers[event_type]:  # Clean up empty list
                    del self.subscribers[event_type]
            except ValueError:
                logger_event_bus.warning(
                    f"Callback {callback.__name__} not found for event type {event_type}"
                )
    ...
    async def publish(self, event: RetailEvent) -> None:
        ...
        if not isinstance(event, RetailEvent):
            logger_event_bus.error(
                f"Attempted to publish invalid event type: {type(event)}"
            )
            return

        logger_event_bus.info(
            f"Event published: {event.event_type} from {event.source.value}"
        )
