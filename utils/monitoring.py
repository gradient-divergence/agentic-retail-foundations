...
class AgentMonitor:
    ...
    def __init__(self, agent_id: str, metric_thresholds: Dict[str, Tuple[float, float]], alert_endpoints: List[Dict[str, str]]):
        """
        Args:
    ...
    def record_metrics(self, metrics_dict: Dict[str, float], timestamp: Optional[datetime] = None):
    ...
    def detect_drift(self, metric: str, window_size: int = 30, change_threshold_percent: float = 15.0) -> bool:
        ...
        try:
            ...
            if previous_avg == 0: # Avoid division by zero
                return bool(recent_avg != 0) # Drift if recent is non-zero and previous was zero
                
            percent_change = abs((recent_avg - previous_avg) / previous_avg) * 100
            return bool(percent_change > change_threshold_percent)
        except Exception as e:
             logger.error(f"Error during drift detection for {metric}: {e}")
             return False
